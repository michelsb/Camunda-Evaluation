...
public class JobWorker {
    ...
    private void registerWorker(String jobType) {
        ...
        JobWorkerBuilderStep1.JobWorkerBuilderStep3 step3 = client.newWorker()
                ...
                .maxJobsActive(config.getMaxJobs())
                .timeout(config.getLockTimeout())
                .requestTimeout(Duration.ofMillis(config.getRequestTimeout()))
                .pollInterval(Duration.ofMillis(fixedBackOffDelay))
                .name(jobType);
            ...
            scheduler.schedule(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        if (markProcessInstanceCompleted && job.getType().contains("-completed")) {
